        ...
        <div class="sv-hero-body">
            ...
            <div class="sv-timestamps">
                @if ($record->submitted_at)
                    <div>
                        <span class="sv-ts-label">{{ __('admin.ts_submitted') }}</span>
                        {{ $record->submitted_at->format('d M Y, H:i') }}
                    </div>
                @endif
                @if ($record->last_edited_at)
                    <div>
                        <span class="sv-ts-label">{{ __('admin.ts_last_edited') }}</span>
        ...
        <div class="sv-release">
            <div class="sv-release-row">
                <div class="sv-release-item">
                    <span class="sv-rlabel">{{ __('admin.rl_form') }}</span>
                    <span class="sv-rvalue">{{ $record->formRelease->form->title }}</span>
                </div>
                <div class="sv-release-item">
                    <span class="sv-rlabel">{{ __('admin.rl_release_set') }}</span>
                ...
                @if ($record->formRelease->releaseSet?->start_at || $record->formRelease->releaseSet?->end_at)
                    <div class="sv-release-item">
                        <span class="sv-rlabel">{{ __('admin.rl_period') }}</span>
                ...
                @endif
                <div class="sv-release-item">
                    <span class="sv-rlabel">{{ __('admin.rl_completion') }}</span>
                    <span class="sv-completion {{ $completePct === 100 ? 'sv-completion--full' : 'sv-completion--partial' }}">
                        {{ $answeredCount }} / {{ $totalCount }} {{ __('admin.rl_answered') }}
                    </span>
                </div>
            </div>
                    ...
                    <div class="sv-q-content">
                        ...
                        <div class="sv-q-header">
                            ...
                            @if ($question->is_required)
                                <span class="sv-required">{{ __('admin.q_required') }}</span>
                            @endif
                        </div>
                        ...
                        @elseif ($question->type === 'checkbox')
                            ...
                            @if (empty($vals) && !$otherText)
                                <span class="sv-no-answer">{{ __('admin.q_no_options_selected') }}</span>
                            @else
                            ...
                            @php
                                ...
                                if (is_array($json) && isset($json['option'])) {
                                    $displayVal = $json['option'] === 'other'
                                        ? __('admin.q_other_prefix') . ($json['other_text'] ?? '')
                                        : $json['option'];
                                } else {
        ...
        @empty
            <div class="sv-card">
                <div class="sv-empty-state">
                    ...
                    <p>{{ __('admin.no_questions_found') }}</p>
                </div>
            </div>
        @endforelse
    </div>
